"""Drop the durable multimodal calibration tables.

Calibration is now processed temporarily and its passed result is stored inside
``medical_interviews.interview_metadata.calibration`` when the interview starts.
The durable ``calibration_attempts`` / ``calibration_media_assets`` lifecycle is
no longer used by any flow, so both tables are dropped.

Revision ID: 028_drop_calibration_tables
Revises: 027_turn_video_analyses
"""

from alembic import op
import sqlalchemy as sa

revision = "028_drop_calibration_tables"
down_revision = "027_turn_video_analyses"
branch_labels = None
depends_on = None
...
def downgrade() -> None:
    # Recreate the durable tables exactly as migration 026 defined them so the
