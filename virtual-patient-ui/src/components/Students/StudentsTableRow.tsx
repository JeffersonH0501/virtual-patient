...
export const StudentsTableRow: FC<StudentsTableRowProps> = ({
  ...
  const handleRowClick = () => {
    navigate(`/clinical-chat/${interview.id}`);
  };
    ...
    >
        ...
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
            <span className="text-xs font-medium text-blue-600">
              {interview.userUsername.charAt(0).toUpperCase()}
            </span>
          </div>
          <span className="font-medium text-gray-900">
            {interview.userUsername}
          </span>
        </div>
      </td>
      ...
      <td className="flex-1 px-6 py-0 text-left text-sm text-gray-500 max-sm:px-3 max-sm:py-0">
        <div className="flex items-center gap-1">
          <span className="font-medium text-gray-900">{interview.evaluationScore || 'N/A'}</span>
          {interview.evaluationScore && <span className="text-xs text-gray-500">/10</span>}
        </div>
      </td>
